<?php
/**
 * Ngo_Charity_Lite Customizer Notification System.
 *
 ...
 */
class Ngo_Charity_Lite_Customizer_Notify {
	...
	/**
	 * The single instance of Ngo_Charity_Lite_Customizer_Notify
	 *
	 * @var Ngo_Charity_Lite_Customizer_Notify $instance The Ngo_Charity_Lite_Customizer_Notify instance.
	 */
	...
	/**
	 * The Main Ngo_Charity_Lite_Customizer_Notify instance.
	 *
	 * We make sure that only one instance of Ngo_Charity_Lite_Customizer_Notify exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Ngo_Charity_Lite_Customizer_Notify ) ) {
			self::$instance = new Ngo_Charity_Lite_Customizer_Notify;
	...
	public function setup_actions() {
		...
		add_action( 'customize_register', array( $this, 'ngo_charity_lite_customizer_notify_customize_register' ) );

		// Enqueue scripts and styles
		add_action( 'customize_controls_enqueue_scripts', array( $this, 'ngo_charity_lite_customizer_notify_scripts_for_customizer' ), 0 );

		/* ajax callback for dismissable recommended actions */
		add_action( 'wp_ajax_ti_customizer_notify_dismiss_recommended_action', array( $this, 'ngo_charity_lite_customizer_notify_dismiss_recommended_action_callback' ) );

		add_action( 'wp_ajax_ti_customizer_notify_dismiss_recommended_plugins', array( $this, 'ngo_charity_lite_customizer_notify_dismiss_recommended_plugins_callback' ) );

	}

	/**
	 * Scripts and styles used in the Ngo_Charity_Lite_Customizer_Notify class
	 */
	public function ngo_charity_lite_customizer_notify_scripts_for_customizer() {
        ...
        wp_localize_script(
			'hl-customizer-notify-customizer-js', 'tiCustomizerNotifyObject', array(
				...
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'ngo-charity-lite' ),
			)
		);
	 ...
	 */
	public function ngo_charity_lite_customizer_notify_customize_register( $wp_customize ) {

		/**
		 * Include the Ngo_Charity_Lite_Customizer_Notify_Section class.
		 */
		require_once get_template_directory() . '/information/class-ngo-charity-lite-customizer-notify-section.php';

		$wp_customize->register_section_type( 'Ngo_Charity_Lite_Customizer_Notify_Section' );

		$wp_customize->add_section(
			new Ngo_Charity_Lite_Customizer_Notify_Section(
				$wp_customize,
	 ...
	 */
	public function ngo_charity_lite_customizer_notify_dismiss_recommended_action_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			if ( get_option( 'ngo_charity_lite_customizer_notify_show_recommended_actions' ) ) {

				$ti_customizer_notify_show_recommended_actions = get_option( 'ngo_charity_lite_customizer_notify_show_recommended_actions' );
				switch ( $_GET['todo'] ) {
					case 'add':
				...
				update_option( 'ngo_charity_lite_customizer_notify_show_recommended_actions', $ti_customizer_notify_show_recommended_actions );
				...
				if ( ! empty( $ti_customizer_notify_recommended_actions ) ) {
					...
					update_option( 'ngo_charity_lite_customizer_notify_show_recommended_actions', $ti_customizer_notify_show_recommended_actions_new );
				}
			}
		}
	 ...
	 */
	public function ngo_charity_lite_customizer_notify_dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			/* if the option exists, update the record for the specified id */
			$ti_customizer_notify_show_recommended_plugins = get_option( 'ngo_charity_lite_customizer_notify_show_recommended_plugins' );
			...
			update_option( 'ngo_charity_lite_customizer_notify_show_recommended_plugins', $ti_customizer_notify_show_recommended_plugins );
		}
		die(); // this is required to return a proper result
	}
